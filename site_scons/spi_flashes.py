...
aTemplateLines = [
    ...
    (
        'Read@ignoreBytes',
        '                    .ucReadOpcodeDCBytes = %d,',
        '/* readOpcodeDCBytes          */'
    ),
    (
        'Write@writeEnableCommand',
        '                    .ucWriteEnableOpcode = 0x%02x,',
        '/* writeEnableOpcode          */'
    ),
    (
        'Erase@erasePageCommand',
        '                      .ucErasePageOpcode = 0x%02x,',
        '/* erase page                 */'
    ),
    (
        'Erase@eraseSectorCommand',
        '                    .ucEraseSectorOpcode = 0x%02x,',
        '/* eraseSectorOpcode          */'
    ),
    (
        'Erase@eraseChipCommandLen',
        '                      .ucEraseChipCmdLen = %d,',
        '/* erase chip command length  */'
    ),
    (
        'Erase@eraseChipCommandHex',
        '                        .aucEraseChipCmd = {%s},',
        '/* erase chip command         */'
    ),
    (
        'Write@pageProgramCommand',
        '                       .ucPageProgOpcode = 0x%02x,',
        '/* pageProgOpcode             */'
    ),
    (
    ...
    (
        ...
        '                    .ucBufferWriteOpcode = 0x%02x,',
        '/* buffer write opcode        */'
    ),
    (
        'Write@eraseAndPageProgramCommand',
        '               .ucEraseAndPageProgOpcode = 0x%02x,',
        '/* eraseAndPageProgOpcode     */'
    ),
    (
        'Status@readStatusCommand',
        '                     .ucReadStatusOpcode = 0x%02x,',
        '/* readStatusOpcode           */'
    ),
    (
        'Status@statusReadyMask',
        '                      .ucStatusReadyMask = 0x%02x,',
        '/* statusReadyMask            */'
    ),
    (
        'Status@statusReadyValue',
        '                     .ucStatusReadyValue = 0x%02x,',
        '/* statusReadyValue           */'
    ),
    (
        'Init0@commandLen',
...
def get_value(tFlashNode, strPath, eType):
    ...
    if len(aPath) == 1:
        ...
        if not aPath[1] in tNode.attrib:
            raise Exception(
                "Node at path %s has no attribute %s" % (
                    aPath[0],
                    aPath[1]
                )
            )
...
def spiflashes_action(target, source, env):
    ...
    for tFlashNode in tXml.findall('SerialFlash'):
        ...
        if strDeviceName in aFlashNames:
            raise Exception(
                'Device %s defined multiple times. '
        ...
        # These commands are optional, replace an empty array with the empty
        # command sequence.
        aOptionalCommands = [
            'Write@writeEnableCommand',
            'Write@pageProgramCommand',
        ...
        # This is hard-coded in the SPI routines. They need to be modified
        ...
        for strPath in aSingleByteCommands:
            ...
            if len(aEntry[strPath]) != 1:
                ...
                raise Exception(
                    'Device %s: The command %s must be a single-byte command. '
                    'Change the SPI code to change this.' % (
                        strDeviceName,
                        strPath
                    )
                )
        ...
        for strPath in aIdEntries:
            if len(aEntry[strPath]) != sizId:
                raise Exception(
                    'Device %s: The ID entries must have the same size!' %
                    strDeviceName
                )
        ...
        for strPath in aHexDumpEntries:
            ...
            aEntry[strPath+'Hex'] = ', '.join(
                ['0x%02x' % ucByte for ucByte in aEntry[strPath]]
            )
        ...
        if strMode not in aLayoutMode:
            raise Exception('Device %s: Unknown layout mode: %s' % (
                strDeviceName,
                strMode
            ))
    ...
    strDefine = (
        '__' +
        os.path.basename(target[1].get_path()).upper().replace('.', '_') +
        '__'
    )
    aReplaceDict = dict({
        'DEFINE':             strDefine,
    ...
    strHeader = string.Template(strHeaderTemplate).safe_substitute(
        aReplaceDict
    )
...
def spiflashes_emitter(target, source, env):
    ...
    target.append(SCons.Script.File(strPath))

    # Depend on this builder.
    env.Depends(target, __file__)
...
def ApplyToEnv(env):
    ...
    spiflashes_act = SCons.Action.Action(
        spiflashes_action,
        spiflashes_string
    )
    spiflashes_bld = SCons.Script.Builder(
        action=spiflashes_act,
        emitter=spiflashes_emitter,
        suffix='.c',
        src_suffix='.xml',
        single_source=1
    )
